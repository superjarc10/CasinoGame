...
public class Cylinder : MonoBehaviour {
    ...
    private List<Element.ElementInfo> config;
    ...
    void UpdateSpinning() {
        ...
        if (spinningDuration <= 0 && didPreviousStop) {
            if (stopCondition.type == CylinderStopCondition.ConditionType.Duration) {
                ...
                if (middleElement.elementInfo.id == stopCondition.middleElementId) conditionTrue = true;
            } else {
                int index = Random.Range(1,4);
                var randomElement = elements[index];
                if (randomElement.elementInfo.id == stopCondition.middleElementId) conditionTrue = true;
            }
        }
        ...
        if (conditionTrue) {
            ...
            endAnim.addOnAnimationEnd(() => {
                // when end animation ends, transition to idle state
                currentState = State.Idle;
                var currentElementIDs = new List<int>();
                currentElementIDs.Add(elements[1].elementInfo.id);
                currentElementIDs.Add(elements[2].elementInfo.id);
                currentElementIDs.Add(elements[3].elementInfo.id);
    ...
    void PutLastElementTop() {
        ...
        lastElementObj.elementInfo = config[currentElementIndex];
    ...
    public void SetupElements(List<Element.ElementInfo> config, int cylinderID) {
        this.config = config;
        ...
        for (int i = 0; i < transform.childCount; i++) {
            ...
            elementObj.elementInfo = config[i];
